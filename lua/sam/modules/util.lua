...
do
	...
	sam.hook_first("PhysgunPickup", "SAM.CanPhysgunPlayer", function(ply, target)
		...
		if (ply:sam_get_nwvar("physgunEnabled", true)) == false and sam.type(target) == "Player" then
			return false
		elseif sam.type(target) == "Player" and ply:HasPermission("can_physgun_players") and ply:CanTarget(target) then
